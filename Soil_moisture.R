...
#Box Plots
smbox2010<-
  ggplot(data=sm.ave10, aes(x=Precip, y=sm))+
  geom_boxplot()+
  scale_y_continuous(name="Soil Moisture",limits = c(-0.01,0.3))+
  scale_x_discrete(name="")+
  ggtitle("2010")


smbox2011<-ggplot(data=sm.ave11, aes(x=Precip, y=sm))+
  geom_boxplot()+
  scale_y_continuous(name="Soil Moisture",limits = c(-0.01,0.3))+
  scale_x_discrete(name="")+
  ggtitle("2011")
# ggsave("SoilMoistureBoxPlot_2011.jpeg", plot=smbox, units = "in", width=3, height=3,dpi=300)
smbox2012<-ggplot(data=sm.ave12nocor, aes(x=Precip, y=sm))+
  geom_boxplot()+
  scale_y_continuous(name="Soil Moisture",limits = c(-0.01,0.3))+
  scale_x_discrete(name="")+
  ggtitle("2012")

boxplots<-grid.arrange(smbox2010, smbox2011, smbox2012, ncol=3)

ggsave("SoilMoistureBoxPlot.jpeg", plot=boxplots, units = "in", width=9, height=3,dpi=300)

##calibrated 2012 data but calibration is wrong

sm.12<-sm.correct%>%
  filter(Probe!="p211",Probe!="p206", Probe!="p204",Probe!="p203", Probe!="p205", Probe!="D7", Probe!="P13", year==2012)
...
sm<-
  ggplot(data=sm.ave12, aes(x=Date, y=sm, group=Precip, color=Precip))+
  geom_point(aes(color=Precip))+
